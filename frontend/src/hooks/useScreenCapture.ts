    ...
    try {
      const response = await fetch('/api/analyze-screen', {
        ...
        body: JSON.stringify({
          ...
          currentMusicPrompt: currentMusicPromptRef.current,
      ...
      const activityDescription = data.activity?.trim() || 'Unknown activity'
      const musicDecision = data.musicDecision?.trim() || 'FALSE'

      setLastDetectedActivity(activityDescription)
      setLastAnalysisTime(new Date().toLocaleTimeString())

      if (musicDecision.toUpperCase() === 'FALSE') {
